...
(i)
\begin{itemize}
\item Less accurate estimation is the penalty paid for using the randomised
response method.
\item We want to test H 0 : \mu A = \mu B against H 1 : \mu A \neq\mu B .
\item 
...
(ii)
(iii)
Distribution of times can be skewed to the right, and we need a log transformation to normalise the data (for test to be valid).
(a)
CI is given by
⎛ s 2 A / s B 2
⎞
...
Page 6
\begin{itemize}
\item The value “1” is included in the 95\% CI, meaning that the assumption of common variance made for the test is valid.
\item SS T = 952.64 – 183 2 /36 = 22.39
\item SS B = (56.1 2 + 59.1 2 + 67.8 2 )/12 − 183 2 /36 = 6.155
\item SS R = 22.39 − 6.155 = 16.235
\end{itemize}
...
\begin{itemize}
\item We have strong evidence against the null hypothesis and conclude that the three mean delay times are not equal.
\item (v) The assumptions are that the data come from normal populations with constant variance.
\item (vi) The plot suggests that the normality assumption is reasonable and that variance does not depend on cause. Test seems valid.
\end{itemize}
\end{document}
