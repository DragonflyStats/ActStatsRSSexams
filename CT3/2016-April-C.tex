...
\begin{enumerate}
7 A random sample is taken from an exponential distribution with parameter λ. The
...
Estimate the parameter λ using the method of maximum likelihood. You are not
required to verify that your answer corresponds to the maximum. [4]
8 A scientist is comparing how productive three new strains of wheat are. Thirty
widely spread plots of equal size are chosen randomly. Ten plots are planted with
each strain and the weight of wheat produced in each plot is measured. The scientist
...
(i) Perform an analysis of variance on the data. [5]
(ii) (a) Determine the width of a 95% confidence interval for the difference
between any two of the mean weights produced for each strain. [4]
The mean weight produced for each sample is:
  Strain 1 2 3
Mean 2.03 4.42 5.24
(b) State which means are significantly different. [1]
The scientist discovers that in practice only ten plots in total were chosen, with a third
of each planted with each strain.
(iii) Comment on what effect this discovery might have on the scientist’s original
analysis above. [2]
[Total 12]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Q7
...
Page 7
(iii) The samples may not be independent of each other and, so, the ANOVA may
not be valid. [2]
[TOTAL 12]
