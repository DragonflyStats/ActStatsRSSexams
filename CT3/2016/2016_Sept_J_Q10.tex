...
\begin{enumerate}
[Total 21]10
A randomised clinical trial was conducted with the aim of investigating the effectiveness of two drugs (A and B) for tackling stage-fright in musicians before a performance. Ten musicians were allocated to each of two groups: group A (taking
drug A) and group B (taking drug B). A further 10 musicians were allocated to a third
group C, which served as a control group with the musicians not taking any drug. All group allocations were random and the musicians did not know which treatment they
received.
At the end of the performance all 30 musicians were asked to give a score indicating their stage-fright, on a scale 1–5, with a score of 1 implying “not nervous at all” and a score of 5 implying “extremely nervous”. The scores were then transformed by
...
6.356
11.589
For these data: SS_{T}= 8.364, SS_{B} = 1.785, SS R = 6.579 (as defined in page 26 of the Formulae and Tables).

\begin{enumerate}

\item (i) Perform an analysis of variance to test the null hypothesis that the mean level of stage-fright is the same among the three groups.
...
There is evidence against the null hypothesis. We conclude that there are differences in the mean level of nervousness scores among the three groups.
...
\begin{itemize}
\item (iv) This interval is too wide. Notice that its validity depends on H_0 being true, which is rejected at 5% level.
\item
(v) We could perform a two-sample t-test of control mean = treatment mean by combining the data for the two treatment groups (and using samples of sizes
10 and 20).
\item 
[Total 17]
