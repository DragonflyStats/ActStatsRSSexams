...
\[(1 12 )(1 ( ) 2 ) − B − \alpha  + \beta B + \alpha \betaB Xt = \varepsilon t\]
...
(i) Explain why s should be 12 in this case (i.e. Yt = Xt – Xt−12). 
 \item (ii) Determine the range of values for \alpha  and \beta for which the process will be
...
autocorrelation values for observations of Yt are \hat{\rho}1 = 0 and \hat{\rho}2 = 0.09.
 \item (iii) Estimate the parameters \alpha  and \beta. 
The actuary observes a sequence of observations x1, x2, …, xT of Xt, with T > 12.
 \item (iv) Derive the next two forecasted values for next two observations \hat{x}_{T}+1 and
\hat{x}_{T} +2, as a function of the existing observations. 
\end{enumerate}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  Q9 
  \begin{itemize}
      \item (i) The first term in the equation has period 12 and so this removes the periodic
effect. 
      \item(ii) The characteristic polynomial will be 1− (\alpha  + \beta) B + \alpha \betaB2 
...
%%-- Page 11
      \item (iii) Yt is an AR(2) where a1 = \alpha  + \beta and a2 = −\alpha \beta. Since from the Yule-walker
...
0.09=\rho2 =a1\rho1+a2 =a2 =\alpha  i.e. \alpha  = −\beta = \pm 0.3. 
      \item (iv) Since 12 t t t Y X X−
= − we have that
XT +1 = YT +1 + XT −11 
XT +2 = YT +2 + XT −10 
With the forecasted values
\hat{x}_T+1} = \hat{y}_T+1} + xT −11 
and
\hat{x}_{T} +2 = \hat{y}_{T} +2 + xT −10 
where
\hat{y}_T+1} = 0* yT + 0.09 yT −1 = 0.09(xT −1 − xT −13 ) 
And similarly
\hat{y}_{T} +2 = 0.09 (xT − xT −12 ) 
[Total 13]
