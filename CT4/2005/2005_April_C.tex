...
level 1.
\item Following a year with two or more claims, move back two levels, or move to
...
\begin{itemize}
    \item As soon as a breakdown occurs an engineer is despatched. \item It can be assumed that the
repair is made immediately, and that it is always possible to repair the machine.
\item The washing machine has never broken down at the start of the year (time t = 0).
\item $P i (t)$ is the probability that the machine has suffered i breakdowns by time t.
\end{itemize}
...
\item And that the boundary condition is satisfied.

\item (iv) Proceeding in a similar way with the equation for P 2 ( t )
P 2 ( t )
1
exp
5
...
1/ 5
3 exp
\item Substituting these values gives:
